...
import warpedrealities.core.gui.subelements.Slider;
import warpedrealities.core.input.Keyboard;
import warpedrealities.core.shared.Callback;
...
public class List extends GUIBase implements Callback {
	...
	public void update(float DT) {
		...
		if (m_delay <= 0 && m_strings != null) {
			if (warpedrealities.core.input.Keyboard.isKeyDown(GLFW.GLFW_KEY_PAGE_DOWN)) {
			...
			if (warpedrealities.core.input.Keyboard.isKeyDown(GLFW.GLFW_KEY_PAGE_UP)) {
				if (m_offset > 0) {
					m_offset--;
					GenFonts();
					m_delay = 0.2F;
